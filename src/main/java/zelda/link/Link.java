...
public class Link extends Karacter {
    ...
    private static final long INPUT_INTERVAL = 50;
    private static final long BOMB_INTERVAL = 3000;
    private static final long ARROW_INTERVAL = 1000;
    private static final long HIT_INTERVAL = 800;
    private static final int MAX_HEALTH = 5;
    private static final int RUPEE_VALUE = 5;
    private static final String LINK_IMAGE_PATH = "images/link.png";
    private static final String BOMB_SOUND_PATH = "sounds/linkHurt.mp3";
    private static final String KILLED_SOUND_PATH = "sounds/killed.mp3";
    private static final String ITEM_SOUND_PATH = "sounds/getItem.mp3";
    ...
    public Link(Game game, int x, int y) {
        super(game, x, y, 17, 20, Direction.DOWN, LINK_IMAGE_PATH);
        z = 2;
        health = MAX_HEALTH;
        screenAdjust = false;
    ...
    public void dropBomb() {
        if (System.currentTimeMillis() > lastBom + BOMB_INTERVAL) {
            switch (direction) {
    ...
    public void shootArrow() {
        if (System.currentTimeMillis() > lastArrow + ARROW_INTERVAL) {
    ...
    protected void collision(GObject hitObject) {
        if (health == 0) {
            ...
            handleArmosKnightCollision((ArmosKnight) hitObject);
        } else if (hitObject instanceof Heart) {
            handleHeartCollision();
        } else if (hitObject instanceof Rupee) {
            handleRupeeCollision();
        }
    }
    ...
    private void handleSoldierCollision(Soldier soldier) {
        if (System.currentTimeMillis() > lastHit + HIT_INTERVAL) {
            game.playFx(BOMB_SOUND_PATH);
            health--;
    ...
    private void handleArmosKnightCollision(ArmosKnight armosKnight) {
        if (System.currentTimeMillis() > lastHit + HIT_INTERVAL) {
            game.playFx(BOMB_SOUND_PATH);
            health--;
            lastHit = System.currentTimeMillis();
    ...
    private void handleHeartCollision() {
        if (health < MAX_HEALTH) {
            game.playFx(ITEM_SOUND_PATH);
            health++;
        }
    }

    private void handleRupeeCollision() {
        game.playFx(ITEM_SOUND_PATH);
        rupee += RUPEE_VALUE;
    }

    public boolean moveInput() {
        return (game.isAPressed()
                || game.isDPressed()
                || game.isWPressed()
                || game.isSPressed());
    }
